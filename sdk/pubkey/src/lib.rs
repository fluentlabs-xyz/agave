...
impl ToPrimitive for PubkeyError {
    ...
    fn to_i64(&self) -> Option<i64> {
        Some(match *self {
            ...
            PubkeyError::InvalidProgramPrefix => PubkeyError::InvalidProgramPrefix as i64,
        })
    }
    #[inline]
...
impl fmt::Display for PubkeyError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            PubkeyError::InvalidProgramPrefix => {
                f.write_str("Provided program id prefix is invalid")
            }
        }
    }
}
...
impl From<u64> for PubkeyError {
    fn from(error: u64) -> Self {
        match error {
            ...
            3 => PubkeyError::InvalidProgramPrefix,
...
impl Pubkey {
    ...
    /// Replacement for address creation for SVM (based on EVM address creation and prefix)
    pub fn create_program_address(
        seeds: &[&[u8]],
        program_id: &Pubkey,
    ) -> Result<Pubkey, PubkeyError> {
        ...
        if seeds.len() <= 0 {
            ...
            salt = hash::keccak256(&seeds_bytes);
        }
        // TODO what to use as a code hash
        let init_code_hash = [0u8; 32];
        // TODO
        let evm_address = calc_create2_address(
            deployer.as_slice()[SVM_ADDRESS_PREFIX.len()..]
                .try_into()
                .unwrap(),
            &salt,
            &init_code_hash,
    ...
    pub fn log(&self) {
        ...
        std::println!("{}", alloc::string::ToString::to_string(&self));
    }
}
    ...
    ($borsh:ident) => {
            ...
            [u8; 32]: $borsh::BorshSchema,
        {
            fn declaration() -> $borsh::schema::Declaration {
                alloc::string::String::from("Pubkey")
            }
            fn add_definitions_recursively(
                ...
                let fields = $borsh::schema::Fields::UnnamedFields(<[_]>::into_vec(
                    alloc::boxed::Box::new([<[u8; 32] as $borsh::BorshSchema>::declaration()]),
                ));
...
//     alloc::string::ToString::to_string(&display).into()
// }
...
//     //             .map_err(|err| JsValue::from(alloc::format!("Invalid Uint8Array pubkey: {err:?}")))
...
//     //                 }
//     //             }
//     //             return Err(alloc::format!("Invalid array argument: {:?}", x).into());
//     //         }
//     //         Pubkey::try_from(bytes)
//     //             .map_err(|err| JsValue::from(alloc::format!("Invalid Array pubkey: {err:?}")))
...
//     pub fn toString(&self) -> alloc::string::String {
...
//     pub fn toBytes(&self) -> alloc::boxed::Box<[u8]> {
//         self.0.clone().into()
//     }
//
